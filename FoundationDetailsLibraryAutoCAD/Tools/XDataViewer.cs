...
namespace FoundationDetailer.AutoCAD
{
    ...
    {
        // QueryNODData keys to check for Xrecords
        ...
        public static void ShowNodXData()
        {
            ...
            if (doc == null)
            {
                MessageBox.Show("No active document.", "QueryNODData XData Viewer", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
            ...
            try
            {
                using (Transaction tr = db.TransactionManager.StartTransaction())
                {
                    ...
                    MessageBox.Show(display.ToString(), "QueryNODData XData Viewer", MessageBoxButton.OK, MessageBoxImage.Information);
            ...
            {
                MessageBox.Show($"Unexpected error: {ex.Message}", "QueryNODData XData Viewer", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        public static void CleanNodHandlesSafe()
        {
            ...
            if (doc == null)
            {
                MessageBox.Show("No active document.", "QueryNODData Cleanup", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
            ...
            try
            {
                using (Transaction tr = db.TransactionManager.StartTransaction())
                {
                    ...
                    foreach (string key in keysToCheck)
                    {
                        ...
                        {
                            // Only upgrade QueryNODData when actually removing an Xrecord
                    ...
                    MessageBox.Show(
                        string.Format("QueryNODData cleanup complete.\nRemoved handles: {0}\nRemoved Xrecords: {1}", removedHandles, removedXrecords),
                        "QueryNODData Cleanup", MessageBoxButton.OK, MessageBoxImage.Information);
                }
            }
            catch (System.Exception ex)
            {
                MessageBox.Show("Error during QueryNODData cleanup: " + ex.Message, "QueryNODData Cleanup", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
